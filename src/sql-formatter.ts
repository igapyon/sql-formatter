...
function createFormatResult(formattedSql, status = 'formatted', stage = null, message = null) {
  return { sql: formattedSql, status, stage, message };
}
  ...
  try {
    ...
    if (!rendered) {
      return createFormatResult(
        ...
        'passthrough',
        'calcite-render',
        'Formatter could not render this SQL, so the original text was returned.',
      );
    }
    if (ctx.unknown) {
      ...
      return createFormatResult(
        ...
        'passthrough',
        'calcite-unsupported',
        'Formatter parsed this SQL only partially, so unsupported parts were left as-is.',
      );
    }
  ...
  try {
    ...
    if (!rendered) {
      return createFormatResult(
        ...
        'passthrough',
        'ddl-render',
        'DDL parser accepted the SQL, but the formatter could not render it.',
      );
    }
    if (ctx.unknown) {
      return createFormatResult(
        ...
        'passthrough',
        'ddl-unsupported',
        'DDL parser accepted the SQL only partially, so the original text was returned.',
      );
    }
    return createFormatResult(rendered);
  } catch (ddlErr) {
    ...
    return createFormatResult(
      ...
      'parse-error',
      'parse',
      `Parse failed. Calcite: ${calciteMessage}. DDL: ${ddlMessage}.`,
    );
  }
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { formatSql, formatSqlWithMeta };
}
