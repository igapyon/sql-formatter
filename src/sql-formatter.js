...
function applyLineComments(rendered, comments) {
    ...
    for (const c of comments){
        ...
        if (idx === -1) {
            for(let i = lines.length - 1; i >= 0; i--){
                const trimmed = lines[i].trim();
                if (trimmed.endsWith(c.anchorRaw)) {
                    idx = i;
                    break;
                }
            }
...
function renderInsert(node, ctx) {
    ...
    if (node.source) {
        const sourceNode = node.source.type === 'OrderedQueryOrExpr' && node.source.query && node.source.query.leaf ? node.source.query.leaf : node.source;
...
//# sourceURL=/Users/igapyon/Documents/git/sql-formatter/src/sql-formatter.ts